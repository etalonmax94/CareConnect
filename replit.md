...
-   **Real-Time Chat System**: Team messaging with direct messages and group chats. Features WebSocket-powered real-time messaging, typing indicators, online/offline presence, read receipts, and message history. Integrated with staff directory for easy conversation creation. Enhanced features include:
    ...
    -   **Message Pinning**: Pin important messages for quick reference with dedicated Pinned Messages panel in chat room settings. Pinned messages shown chronologically with unpin capability.
    -   **Voice Messages**: Record and send voice messages using MediaRecorder API. Displays audio playback widget with waveform visualization. 60-second maximum recording with real-time duration display.
    -   **Message Scheduling**: Schedule messages to be sent at a future date/time. Scheduled messages queue displayed in composer with ability to cancel before send time. Scheduler runs server-side to deliver messages at scheduled times.
    -   **Chat Export**: Export chat history as formatted text file (.txt) with timestamps, sender names, and message content. Export button in chat room settings.
    -   **Advanced Message Search**: Filter messages by sender, date range, and media type (text, images, videos, audio, documents). Search panel with quick filters and paginated results.
    -   **Quick Emoji Reactions**: Hover-based floating reaction picker showing common emojis (heart, thumbsup, thumbsdown, diamond, party) for one-click reactions without opening menus.
    -   **Chat Templates**: Save and reuse common messages with localStorage-based template storage. Template management dialog with create/edit/delete functionality. Insert templates into message composer with one click.
    -   **Enhanced Typing Indicators**: Show which specific staff members are typing with avatars instead of generic "typing..." indicator.
    -   **Delivery Receipts**: Track message delivery status with visual indicators - single check (sent), double check (delivered), blue double check (read). Uses `chat_message_deliveries` table with batch processing endpoints.
    -   **Shared Media Gallery**: View all photos and documents shared in chat room with 30-day retention indicator.
    -   **Client Quick Access**: Quick access buttons in client-type chat headers linking to client profile, care plan, and documents.
    -   **Duplicate Chat Prevention**: Automatically reuses existing chat rooms when same participants try to create a new conversation.
    -   **Enhanced Delete Messages**: Regular users can delete own messages within 2 minutes; Admins can delete any message within 24 hours. Shows "Message was deleted" placeholder with deleter info.
